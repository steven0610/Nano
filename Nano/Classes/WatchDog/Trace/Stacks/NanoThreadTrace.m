 ...
 *  @param callstack    caller space for saving stack trace info
...
int nanoThreadCallStacks(thread_t thread, void** callstack, int maxCount) {
    ...
    if (machineContext.__ss.__framePointer) {
        ...
        NSLog(@"You should check the compare info of the project. And open the framePointer");
    }

    return i;
}

#pragma mark - deal Thread
NSString* traceForThread(thread_t thread) {
    ...
    /* 优化 count 算法 */
    int backtraceCount = nanoThreadCallStacks(thread, backtraceStack, maxStackDepth);
